...
PROGRAM test
    ...
    INTEGER :: nx, ny, nz, i, j,k, z, schritt, Q, ubalken,zq,xq,yq,h
    REAL    :: fg,fk,gg,gk,dy,dx,dey,dez,Pi
    ...
    DO i = 0, nx
       DO j = 0, ny
        DO k = 0, nz
       	  dey = fg*i**fk
       	  dez =gg*i**gk
          c(i,j,k) =(Q/(2* Pi*dey*dez*ubalken))*EXP((-j**2)/(2*dey**2)) *(EXP((-(k-h)**2)/(2*dez**2)) +EXP((-(k+h)**2)/(2*dez**2)))
        ENDDO
       ENDDO
    ENDDO
